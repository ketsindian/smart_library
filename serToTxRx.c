...
int main(int argc, char *argv[])
{
     ...
     fd = open("/dev/ttyUSB7", O_RDWR | O_NOCTTY | O_NDELAY);
     if (fd == -1)
     {
	...
	fp2=fopen("/home/katty/Desktop/GSM.txt","r");
...
void signal_handler_IO (int status)
{
    ...
    // printf("received data from UART.\n");
     ...
     //{       
               //printf("received data from UART.\n");
        res = read(fd,&buf[i],13);//#change 255		//as an extra charachter is transmitted
	//printf("from ISR %s \n",buf);
              ...
              //printf(":%s", buf, res);
               ...
               {
                    i=0;
                    fp=fopen("/home/katty/Desktop/RFID.txt","w");
                    //printf("%d\n",fp);
                    if(fp==NULL)
                         printf("\nfile not created\n");
                    //fprintf(fp,"%s",s1); 
